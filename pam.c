...
#define PAM_SERVICE_NAME "opunra"

static pam_handle_t *pamh = NULL;
static char opunra_prompt[128];
...
pamauth(const char *user, const char *myname, int interactive, int nopass, int persist)
{
	...
	if (!nopass) {
		...
		/* opunra style prompt for pam */
		char host[HOST_NAME_MAX + 1];
		if (gethostname(host, sizeof(host)))
			snprintf(host, sizeof(host), "?");
		snprintf(opunra_prompt, sizeof(opunra_prompt),
		    "\ropunra (%.32s@%.32s) password: ", myname, host);
